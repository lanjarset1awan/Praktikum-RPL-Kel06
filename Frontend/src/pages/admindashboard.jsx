...
function AdminDashboard() {
            ...
            <tbody>
              {currentReports.map(r => (
                <tr key={r.id}>
                  ...
                  <td>
                    <a href="#" className="btn-link" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontSize: '0.75rem', lineHeight: 1.2 }} onClick={(e) => { e.preventDefault(); setSelectedReport(r); setActiveView('detail'); }}>
                      Lihat<br/>Detail <i className="fas fa-chevron-right" style={{ fontSize: '0.7rem', marginTop: '2px' }}></i>
                    </a>
                  </td>
                </tr>
              ))}
            ...
            <div className="detail-card">
              <div className="detail-top">
                <div>
                  ...
                  <div className="detail-meta">Dilaporkan pada: {formatTanggal(selectedReport.createdAt)}</div>
                </div>
                ...
                {showStatusDropdown && (
                  <div className="status-dropdown-menu">
                    ...
                    <div className="status-dropdown-item" onClick={() => handleUpdateStatus("diproses")}>
                      <span>Diproses</span>
                      <div className="status-dot" style={{ backgroundColor: '#0ea5e9' }}></div>
                    </div>
                    ...
                    <div className="status-dropdown-item" onClick={() => handleUpdateStatus("pending")}>
                      <span>Pending</span>
                      <div className="status-dot" style={{ backgroundColor: '#94a3b8' }}></div>
                    </div>
                  </div>
                )}
              </div>
  ...
  const renderStatistikLaporan = () => {
    ...
    const berat = reports.filter(r => r.category?.toLowerCase() === "berat").length;
    const sedang = reports.filter(r => r.category?.toLowerCase() === "sedang").length;
    const ringan = reports.filter(r => r.category?.toLowerCase() === "ringan").length;
